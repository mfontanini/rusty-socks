...
use tokio::io::{split, AsyncRead, AsyncWrite, BufReader, BufWriter, ReadBuf, ReadHalf, WriteHalf};
...
impl AsyncRead for Stream {
    fn poll_read(
        ...
        match self.get_mut().stream_type {
            StreamType::Tcp(ref mut reader, _) => AsyncRead::poll_read(Pin::new(reader), cx, buf),
...
impl AsyncWrite for Stream {
    fn poll_write(
        ...
        match self.get_mut().stream_type {
            StreamType::Tcp(_, ref mut writer) => AsyncWrite::poll_write(Pin::new(writer), cx, buf),
    ...
    fn poll_flush(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<io::Result<()>> {
        match self.get_mut().stream_type {
            StreamType::Tcp(_, ref mut writer) => AsyncWrite::poll_flush(Pin::new(writer), cx),
            StreamType::BufferedTcp(_, ref mut writer) => {
    ...
    fn poll_shutdown(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<io::Result<()>> {
        match self.get_mut().stream_type {
            StreamType::Tcp(_, ref mut writer) => AsyncWrite::poll_shutdown(Pin::new(writer), cx),
            StreamType::BufferedTcp(_, ref mut writer) => {
